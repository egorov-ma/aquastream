...
**Критерии готовности**
- Проект собирается и запускается (dev/prod), все страницы‑заглушки доступны.
- Лайт/дарк темы переключаются; линтеры и форматтеры настроены.

### Спринт 2 — Публичный каталог
**Задачи**
- Главная: список организаторов, поиск, пагинация.
- Страницы организатора: главная, события (список + фильтры), команда, FAQ.
- `OrganizerCard`, `EventCard`, состояния loading/empty/error.
- SSG/ISR (60с) и синхронизация фильтров с URL.

**Критерии**
- Каталог и страницы организатора работают, фильтры изменяют URL и список.

### Спринт 3 — Аутентификация и профиль
**Задачи**
- Регистрация/логин (username+password), cookie‑сессии.
- Профиль: телефон, Telegram, доп. поля; верификация Telegram (UI).
- RBAC‑гварды и редиректы, экран уведомлений (опциональные подписки).
- Восстановление доступа: через Telegram/резервные коды (UI+эндпойнты).

**Критерии**
- Без заполненного профиля — запись на событие недоступна.
- Настройки уведомлений сохраняются.

### Спринт 4 — Бронирование и оплата (виджет + QR)
**Задачи**
- Создание `Booking` и страница `/checkout/[bookingId]`.
- Интеграция **виджета** (по умолчанию YooKassa): init + success/cancel.
- Route handler под вебхук (стаб на dev), смена статусов брони.
- QR‑поток: загрузка пруфа, просмотр и модерация организатором.
- Тосты/баннеры статусов, минимальные логи ошибок.

**Критерии**
- Happy‑path оплаты через виджет завершает бронь в `paid`.
- QR‑оплата подтверждается вручную и меняет статус. Телеграм‑уведомления уходят.

### Спринт 5 — Кабинет организатора
**Задачи**
- CRUD событий (черновик → предпросмотр → публикация).
- Редактор бренда: логотип/описание/контакты; редакторы команды и FAQ.
- Экипажи/лодки/палатки: CRUD простых списков, назначение участников.
- Модерация оплат (accept/reject).

**Критерии**
- Организатор может создать и опубликовать событие и управлять составом.

### Спринт 6 — Админ, waitlist и стабилизация
**Задачи**
- Экран ролей (назначение/снятие).
- Waitlist (минимум) + уведомление при освобождении места.
- Playwright smoke‑набор, базовый A11y/перф‑проход, Sentry (prod).
- Документация: README (запуск, env, платежи, вебхуки).

**Критерии**
- Роли управляются из админки; waitlist работает; smoke‑тесты зелёные.
- MVP готов к ограниченному прод‑запуску.

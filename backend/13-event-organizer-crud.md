# T13. Event: CRUD событий (организатор) и публикация
...
## Цель
Дать организатору CRUD по событиям и публикацию (draft→published).

## Результат (Deliverables)
- `POST /events`, `PUT /events/{id}`, `POST /events/{id}/publish`
- Валидации dates/capacity/price

## Шаги
1. Проверка владения ресурсом (организатор).
2. Переход статусов с аудитом; блокировать конфликтные операции.

## Критерии приёмки (AC)
- Неверные переходы → `409` Problem Details.

## Definition of Done (DoD)
- Таблица статусов и переходов в README.

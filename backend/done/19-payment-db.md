# T19. Payment: схема БД и миграции Liquibase
...
## Цель
Описать схему `payment` (платежи/пруфы/вебхуки с идемпотентностью).

## Результат (Deliverables)
- Таблицы: `payments`, `payment_receipts`, `webhook_events` (idempotency key уникален)

## Шаги
1. Ввести уникальный индекс на идентификатор события провайдера.
2. Поля статусов: pending/submitted/succeeded/rejected/canceled.

## Критерии приёмки (AC)
- Дубликаты вебхуков ловятся на уровне БД.

## Definition of Done (DoD)
- Документация статусов переходов.

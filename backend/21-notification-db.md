# T21. Notification: схема БД и миграции Liquibase
...
## Цель
Схема `notification` для prefs/линковок Telegram/outbox.

## Результат (Deliverables)
- Таблицы: `notification_prefs`, `telegram_subscriptions`, `outbox`

## Шаги
1. Описать связи на `user.users`; индексы по `user_id` и `telegram`.

## Критерии приёмки (AC)
- Миграции применяются; ограничения целостности соблюдены.

## Definition of Done (DoD)
- Документация категорий уведомлений (required/optional).

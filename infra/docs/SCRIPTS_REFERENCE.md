...
- `--use-env` — использовать переменные окружения вместо `infra/docker/compose/.env`. Секреты рекомендуется хранить в GitHub Secrets.
...
**Результат:** Новые сильные пароли сгенерированы и сохранены в .env (или экспортированы в окружение при использовании `--use-env`)
